...
// 		첫번째 매개변수로 내장객체인 request를 사용, 두번째 매개변수로 파일이 저장될 경로를 설정(해당 폴더가 없을 경우 오류가 발생함)
// 		세번째 매개변수로 업로드할 파일의 최대 크기를 설정(여기서 사용된 1024는 컴퓨터가 2진수로 계산하므로 1000을 2의 10승으로 계산하여 때문임)
// 		네번째 매개변수로 사용할 문자셋을 설정하고 마지막 매개변수로 중복 파일의 이름 변경 규칙을 설정함
// 	자바에서 각종 폴더 및 파일의 경로를 문자열로 입력 시 \\를 2번 이유는 '\' 가 이스케이프 문자이기 때문에 \ 1개만 사용 시 이스케이프 문자로 동작하고 \\을 사용해야 문자인 \ 로 인식하기 때문임
	MultipartRequest multi = new MultipartRequest(request, "C:\\java404\\upload", 5 * 1024 * 1024, "utf-8", new DefaultFileRenamePolicy());
	
// 	MultipartReqeust 클래스 타입의 객체에서 getParameterNames()를 사용하여 클라이언트에서 전송한 텍스트 데이터의 이름을 모두 가져옴
...
// 	가져온 텍스트 데이터를 반복문을 통해서 출력
	while (params.hasMoreElements()) {
		...
		String value = multi.getParameter(name); // 지정된 이름과 1:1로 매칭된 데이터 가져오기
